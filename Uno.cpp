...
int defaultPoints = 500;           // the number of points needed to end game is automatically 500 points

// enumeration is a user-defined type (in this case "Color") consisting of a set of named integral constants (in this case card colors)
...
Color StrToColor(string c) {
	if (c == "red")  // if you only have one statement, you don't actually need brackets
		return RED;
	else if (c == "blue")
		return BLUE;
	else if (c == "green")
...
// A class is a user-defined data type (the class below is creating a "Card" object). We use the "Card" object 
// throughout our code (because UNO is a card game of course!).
class Card {
    //FIELDS
	...
	bool isWildDraw4() {    // function checking if the card a wild draw 4
		if (this->GetAction() == Act_WildDraw4) // 'this' pointer is needed because Action is in private
	...
	string WhatCardIsThis(bool checking) {    // a function that interprets the card's color and allows it to later be output as a string
		string cardName;
		if(checking == false){
      if(this->GetColor() == RED) {    // colors
			  cardName = "\033[1;31mred ";
		  }	else if(this->GetColor() == YELLOW) {
    		cardName = "\033[1;33myellow ";
		  }	else if(this->GetColor() == BLUE) { 
    		cardName = "\033[1;34mblue ";
		  }	else if(this->GetColor() == GREEN) {
    		cardName = "\033[1;32mgreen ";
  		} else {
        cardName = "\033[1;35m";
      }
    }else{
  ...
  if(checking == false)
    return cardName + "\033[0m";  // return a string with card name
  else
    return cardName;
}
  ...
  vector<Card> GetHand() {	// creates hand vectors containing Card objects
	...
	void DisplayHand() {
		...
		for (int j = 0; j < GetHand().size(); ++j){
			Color whatColor = GetHand().at(j).GetColor();

			if (whatColor == GREEN) {   // when player's hand is displayed, green cards are shown first
				haveGreen = true;
			}
			else if (whatColor == BLUE) {   // blue cards are displayed second
				haveBlue = true;
			}
			else if (whatColor == YELLOW) { // yellow cards are displayed third
				haveYellow = true;
			}
			else if (whatColor == RED) {    // red cards are displayed fourth
				haveRed = true;
			}
			else if (whatColor == WILD) {   // and wildcards are displayed fifth
				haveWild = true;
			}
		}
		cout << endl;
		if (haveRed == true) {  // red cards in a player's hand are output in red
			for (int j = 0; j < GetHand().size(); ++j){
				if (GetHand().at(j).GetColor() == RED)
					cout << GetHand().at(j).WhatCardIsThis(false) << ", ";
			}
		}
		if (haveGreen == true) {    // green cards in a player's hand are output in green
			for (int j = 0; j < GetHand().size(); ++j){
				if (GetHand().at(j).GetColor() == GREEN)
					cout << GetHand().at(j).WhatCardIsThis(false) << ", ";
			}
		}
		if (haveYellow == true) {   // yellow cards in a player's hand are output in yellow
			for (int j = 0; j < GetHand().size(); ++j){
				if (GetHand().at(j).GetColor() == YELLOW)
					cout << GetHand().at(j).WhatCardIsThis(false) << ", ";
			}
		}
    	if (haveBlue == true) { // blue cards in a player's hand are output in blue
			for (int j = 0; j < GetHand().size(); ++j){
				if (GetHand().at(j).GetColor() == BLUE)
					cout << GetHand().at(j).WhatCardIsThis(false) << ", ";
			}
		}
		if (haveWild == true) { // wild cards in a player's hand are output in magenta
			for (int j = 0; j < GetHand().size(); ++j){
				if (GetHand().at(j).GetColor() == WILD)
					cout << GetHand().at(j).WhatCardIsThis(false) << ", ";
			}
			cout << "\033[0m";
		}
...
class Deck {    // creating the deck
	...
	Card Draw() {    // draw the top card off the now shuffled cards deck
		...
		// cout << " Card Drawn: " << n.WhatCardIsThis(false) << endl;   // (when uncommented, displays all the cards drawn)
...
void SelectWildColor() {
	bool colorWrong = true;
		while (colorWrong == true) {
			cout << "What color would you like to change to ( " << "\033[1;31m red" << "\033[0m ," << "\033[1;32m green";
			cout << "\033[0m ," << "\033[1;33m yellow" << "\033[0m , or" << "\033[1;34m blue" << "\033[0m ): ";
			cin >> cardColorPlayed;
...
bool validTop = false;
	...
	if (Top.isWildCard()) {
				cout << endl << "Top card is " << "\x1b[35m" << "wild" << "\x1b[0m" << endl << ". color needs to be selected.";
				cout << endl << "First Player's Hand contains: ";
    			players.at(playerTurn).DisplayHand();
    			cout << endl;
		SelectWildColor();
		}
    ...
    skip = false;
				...
				cout << "Top card is " << discardPile.at(0).WhatCardIsThis(false);
				if ((discardPile.at(0).isWildCard()) || (discardPile.at(0).isWildDraw4())){
					if (cardColorPlayed == "red")
                        cout << " [" << "b[31m" << cardColorPlayed << "\x1b[0m" << "]" ;
                    else if (cardColorPlayed == "green")
                        cout << " [" << "\x1b[32m" << cardColorPlayed << "\x1b[0m" << "]" ;
        			else if (cardColorPlayed == "yellow")
                        cout << " [" << "\x1b[33m" << cardColorPlayed << "\x1b[0m" << "]" ;
        			else if (cardColorPlayed == "blue")
                        cout << " [" << "\x1b[34m" << cardColorPlayed << "\x1b[0m" << "]" ;
				}
	...
	if (firstloop == 1) {
        ...
        getline(cin, playedCard);
            ...
            for(ind = 0; ind < players.at(playerTurn).GetHand().size(); ind++) {
                if(players.at(playerTurn).GetHand().at(ind).WhatCardIsThis(true) == playedCard) {
        		foundCard = true;
        		break;
                }
            }
        ...
        // debug
			...
			continue;   // "continue" is a loop control statement forces the loop to continue or execute the next iteration
