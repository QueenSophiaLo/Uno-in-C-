...
class Card {
    ...
    public:
        ...
        int GetCardPoints(){							// getter method- returns private data stored in an object's member variables
...
class Player{
  ...
  vector<Card> GetHand(){		// creates object holding hand vectors
	...
	int CalculatePoints() {
		...
		cout << plyrName << " number of cards: " << numCards << endl;
		for (int i = 0; i < numCards; ++i){								// For loop removing top card from hand, add card's points to "points"
			...
			//total += GetHand().back().GetCardPoints();	// This will clear the player's hand and keep track of points
			//GetHand().pop_back();
		}
		for (int i = 0; i < numCards; ++i){
			GetHand().pop_back();
		}
		cout << plyrName << " points: " << total << endl;
		return total;
	}
...
class Deck{
	...
	void shuffle() {													// a function to shuffle the deck
		...
		// warning: if there are less than 108 cards, this does not work.  Reset 
	...
	Card Draw(){														// draw the top card off the now shuffled cards deck
		...
		cout << "  Card Drawn: " << n.WhatCardIsThis() << endl;
...
int PlayerHas500Points(vector<Player> players){
	for (int i = 0; i < players.size(); ++i){
		cout << " Player: " << players.at(i).GetName() << "  points: " << players.at(i).GetPoints() << endl;
	}
	for (int i = 0; i < players.size(); ++i){
...
// shuffle the deck
		// Reset the deck so it has 108 cards
		deck_of_cards.Reset();
...
//cout << "player direction3 " << plyrMotion<< endl;
//cout << "skip=" << skip << endl;
