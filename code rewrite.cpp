...
*/
void PlayerHas500Points(vector<int> playerpoints){
...
// player 1 starts
CanCardBePlayed(Card topCard);		// FIXME needs to check to see if player has a playable card
ReverseCardPlayed(FIXME current player, Card topCard);
SkipCardPlayed(FIXME current player, Card topCard);
DrawTwoCards(FIXME next players hand, Card topCard, bool skip = false);
WildCardPlayed(Card topcard);
...
// winning parameters
PlayerHasZeroCards(FIXME current players hand, current players name, Card topCard);
